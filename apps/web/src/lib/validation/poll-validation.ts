...
export const validateField = (field: keyof PollFormData, value: any): string | null => {
  switch (field) {
    case 'optionAImage':
    ...
    case 'optionALabel':
      return value && value.length > 50 ? 'Option A label must be 50 characters or less' : null;
    case 'optionBLabel':
      return value && value.length > 50 ? 'Option B label must be 50 characters or less' : null;
    case 'description':
      return value && value.length > 500 ? 'Description must be 500 characters or less' : null;
    default:
      return null;
  }
};
