...
export class AnalyticsService {
  ...
  static async trackPollAccess(pollId: string, referrer?: string): Promise<void> {
    try {
      const supabase = createClient();
  ...
  static async getPollAnalytics(pollId: string): Promise<PollAnalytics | null> {
    try {
      ...
      const pollEndDate = new Date(votes[votes.length - 1]?.created_at || new Date());
      const daysDiff = Math.max(1, Math.ceil((pollEndDate.getTime() - pollStartDate.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  static async getTopSharedPolls(limit: number = 10): Promise<Array<{
    pollId: string;
    pollTitle: string;
    shareCount: number;
    accessCount: number;
    ...
    try {
      ...
      const accessCounts = access?.reduce((acc, access) => {
