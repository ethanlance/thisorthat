...
describe('PollCreationForm', () => {
  ...
  it('renders the form with all required fields', () => {
    ...
    expect(screen.getByText('Upload two images and let your friends help you decide!')).toBeInTheDocument();
    expect(screen.getByTestId('image-upload-a')).toBeInTheDocument();
    expect(screen.getByTestId('image-upload-b')).toBeInTheDocument();
    expect(screen.getByLabelText('Option A Label (optional)')).toBeInTheDocument();
    expect(screen.getByLabelText('Option B Label (optional)')).toBeInTheDocument();
  ...
  it('handles form submission errors', async () => {
    (PollsService.createPoll as any).mockRejectedValue(new Error('Database error'));
  ...
  it('validates field length limits', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Option B label must be 50 characters or less')).toBeInTheDocument();
    });
  ...
  it('handles image upload errors', async () => {
    ...
    const optionAErrorButton = screen.getByTestId('image-upload-a').querySelector('button:last-child');
