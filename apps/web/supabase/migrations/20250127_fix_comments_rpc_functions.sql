-- Fix Comments RPC Functions
-- This migration fixes the RPC functions to work properly with RLS

-- Drop and recreate the get_comment_thread function with better error handling
DROP FUNCTION IF EXISTS get_comment_thread(UUID, INTEGER, INTEGER);
...
BEGIN
  ...
  SELECT 
    ...
    c.updated_at,
    COALESCE(
      (SELECT display_name FROM auth.users WHERE id = c.user_id),
      (SELECT email FROM auth.users WHERE id = c.user_id),
      'Anonymous'
    ) as user_display_name,
    (SELECT avatar_url FROM auth.users WHERE id = c.user_id) as user_avatar_url,
...
-- Drop and recreate the get_comment_replies function
DROP FUNCTION IF EXISTS get_comment_replies(UUID, INTEGER, INTEGER);
...
GRANT EXECUTE ON FUNCTION get_comment_thread(UUID, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_comment_replies(UUID, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION update_comment_reaction(UUID, TEXT) TO authenticated;
