...
## Tasks / Subtasks
- [ ] Task 1: Native Sharing Integration (AC: 1, 2, 3)
  - [ ] Implement Web Share API for native mobile sharing
  - [ ] Add clipboard copy functionality with one-tap
  - [ ] Create social media sharing options
  - [ ] Handle sharing across different platforms
- [ ] Task 2: Link Generation and Validation (AC: 4, 5)
  - [ ] Generate unique, shareable poll URLs
  - [ ] Ensure links work without app installation
  - [ ] Add link validation and error handling
  - [ ] Implement deep linking for mobile apps
- [ ] Task 3: Poll Preview and Metadata (AC: 6)
  - [ ] Create Open Graph meta tags for poll previews
  - [ ] Add Twitter Card support for social sharing
  - [ ] Generate poll preview images
  - [ ] Implement dynamic meta tag generation
- [ ] Task 4: Analytics and Tracking (AC: 7)
  - [ ] Track poll link clicks and access
  - [ ] Display access statistics to poll creators
  - [ ] Add referral tracking for shared links
...
export class AnalyticsService {
  ...
  static async getPollAnalytics(pollId: string): Promise<PollAnalytics> {
    ...
    return {
      ...
      shareMethods: shares?.reduce((acc, share) => {
        acc[share.method] = (acc[share.method] || 0) + 1;
        return acc;
      }, {} as Record<string, number>) || {},
      referrers: access?.reduce((acc, access) => {
